...
2. `окрашены, был кератин` → вопрос про филиал
3. `Жамбыла` → вопрос про мастера (или сразу про дату/время, если мастер один)
4. `Анна` → вопрос: удобные дата и время
5. `завтра в 14:00` → вопрос про имя (клиент сам называет время, окон-номеров нет)
6. `Айгерим` → вопрос про телефон
7. `+7 707 123 45 67` → закрытие «Вы записаны … Администратор перезвонит»
